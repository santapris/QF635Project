...
from trading.config import BinanceOrderGatewaySpec, load_config
...
def load_runner_config(*, require_credentials: bool, futures: bool) -> RunnerConfig:
    ...
    spec = next(
        (g for g in cfg.order_gateways if isinstance(g, BinanceOrderGatewaySpec)),
        None,
    )
    if spec is None:
        raise ConfigError(
    ...
    missing = [
        name for name in ("spot_rest_base", "spot_ws_base",
                          "futures_rest_base", "futures_ws_base")
        if getattr(spec, name) is None
    ]
    if missing:
        raise ConfigError(
            f"{_DEFAULT_CONFIG.name} must set {missing} on the Binance gateway",
            path=str(_DEFAULT_CONFIG),
        )

    binance = BinanceConfig(
        spot_rest_base=spec.spot_rest_base,
        spot_ws_base=spec.spot_ws_base,
        futures_rest_base=spec.futures_rest_base,
        futures_ws_base=spec.futures_ws_base,
        futures=futures,
    )

    credentials: BinanceCredentials | None = None
    if require_credentials:
        key = os.environ.get(f"{spec.credentials_env}_API_KEY")
        secret = os.environ.get(f"{spec.credentials_env}_API_SECRET")
        if not key or not secret:
            raise ConfigError(
                f"missing Binance credentials; set {spec.credentials_env}_API_KEY "
                f"and {spec.credentials_env}_API_SECRET in the environment"
            )
