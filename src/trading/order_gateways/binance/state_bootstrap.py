...
class StateBootstrapper:
    ...
    async def _resync_once(self) -> None:
        ...
        The "gone from venue" terminalize is scoped to instruments whose
        ``/openOrders`` GET succeeded this pass (``fetched_iids``) — a failed GET
        means we know nothing about that symbol's venue state and must change
        nothing, or we would terminalize live orders on a network blip.
        """
        seen_coids, fetched_iids = await self._adopt_open_orders()

        # Terminalize any locally-open order the venue no longer reports — but
        # only on instruments we actually fetched.
